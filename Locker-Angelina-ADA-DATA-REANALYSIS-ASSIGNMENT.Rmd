...
ggplot(data = d, aes(d$Average.18O.canines, na.rm = TRUE)) + 
...
ggplot(data = d, aes(d$Average.18O.molars, na.rm = TRUE)) + 
  geom_histogram(aes(y = ..density..), 
                 na.rm = TRUE, 
                 breaks = seq(0, 12, by = 0.5),
                 col = "red",
                 fill = "red") +
  labs(x = "Third Molar Average"~delta*'18O(\u2030PDB)', y = "Frequency") +
  xlim(c(-7.0, -1.0)) + 
  ylim(c(0,12))
...
```{r}
par(mfrow = c(1,2))
qqnorm(d$Average.18O.canines) #QQ plots are used to for a normal distribution. Wright's axes are switched.
qqline(d$Average.18O.canines)

qqnorm(d$Average.18O.molars)
qqline(d$Average.18O.molars)
```
